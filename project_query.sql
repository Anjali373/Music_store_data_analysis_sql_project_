...
-- Q5] who is the best customer? The customer who has spent the most money will
    -- be declared the best cutomer. Write a query that returns the person who
...
-- Q7] Let's invite the artist who have written the most rock music in our dataset.
    -- Write a query that returns the artist id and total track count of the top
	-- 10 rock bands.

select artist.artist_id ,count(artist.artist_id) 
from track
...
select person_name,milliseconds
from track
where milliseconds > (
...
-- Q9] Find how much amount spent by each customer on artist? Write a query to return
    -- customer name, artist name amd total spent.
...
-- Q10] We want to find out the most popular music genre for each country. We determine the
    -- most popular genre as the genre with the highest amount of purches.Write a query
...
-- Q11] write a query that determine the customer that has spent the most on music for each country.
    --  Write query that returns the country along with the top customers and how much they spent.
